...
export const nameValidation = {
  ...
  validate: {
    minLength: (value: string) =>
      value.trim().length >= 3 || 'Use a minimum of 3 characters.',
    maxLength: (value: string) =>
      value.trim().length <= 12 || 'Use a maximum of 12 characters.',
    hasSpecialChar: (username: string) => {
      return (
        !containsSpecialChar.test(username) ||
...
export const passwordValidation = {
  ...
  validate: {
    hasBigLetter: (password: string) =>
      containsCapitalLetter.test(password) || 'Atleast one capital letter',
    hasSpecialChar: (password: string) =>
      containsSpecialChar.test(password) || 'Atleast one special character.',
  },
};
export const generalInvoiceValidation = {
  required: 'Cannot be empty',
  validate: {
    minLength: (value: string) => value.trim().length > 0 || 'Cannot be empty',
  },
};
